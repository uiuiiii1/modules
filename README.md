...
| OLED 显示屏 | `OLED/`（HAL 库版）/ `OLED2/`（标准库版） | HAL 库 / 标准库 | 128×64 IIC 接口 SSD1306 显示屏驱动，两个版本功能一致 |
...
**编译设置**：需要在 Keil 的 **C/C++** 部分 → **Misc Controls** 加上 `--no-multibyte-chars`，否则汉字字模显示会乱码（两个版本 `OLED/` 和 `OLED2/` 都需要）。

**参考**：江科大自化协 STM32 教程。

**用法**：把对应模块的 `.c/.h` 拷入你的工程，按注释初始化即可。`OLED_Data.c` 需一并拷入（含字模数据）。

**库**：STM32 HAL 库 / STM32 标准外设库

**两个版本**：`OLED/` 为 HAL 库版，`OLED2/` 为标准库版。两个版本功能与函数接口完全一致，仅软件库不同，按需选用。
